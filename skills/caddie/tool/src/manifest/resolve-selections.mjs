...
export async function resolveSelectionsWithEvidence(manifest, {
  ...
  for (const selection of manifest.skills) {
    ...
    if (inspected.findings.some(isSkillConformanceFinding)) continue;
...
function validatedSkillMetadata(content, skillFile, expectedDirectoryName) {
  ...
  if (metadata.standardFindings.length > 0) {
    throw invalid('skill-metadata-invalid', `SKILL.md does not conform to the Agent Skills specification: ${skillFile}`, {
      skillFile, findings: metadata.standardFindings,
    });
  }
  const name = requireSkillName(metadata, skillFile);
  if (expectedDirectoryName !== null && name !== expectedDirectoryName) {
    throw invalid('skill-name-directory-mismatch', `SKILL.md name does not match its directory: ${skillFile}`, {
...
function isSkillConformanceFinding(finding) {
  return finding?.code?.startsWith('skill-')
    || ['missing-skill-file', 'unreadable-skill-file'].includes(finding?.code);
}
