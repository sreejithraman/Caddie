...
1. Resolve `tool/caddie.mjs` relative to the directory containing this `SKILL.md`, then run it with Node and one versioned JSON request on standard input. Start with `locate`, then use `inspect`, `inspect-source`, or `compare` for the user's question.
...
Use the canonical terms in [references/domain.md](references/domain.md). Preserve unknown, modified, colliding, and permission-blocked content. Adoption treats legacy manager state as evidence. Unmanagement preserves Materialized Skills and Agent Harness exposure; destructive cleanup is a separate approved plan.
