...
test('inspect composes User Skills and Project Skills and reads names from SKILL.md', async () => {
  ...
  await skill(path.join(userSource, 'shared'), 'shared-helper');
  await skill(path.join(projectSource, 'focused'), 'project-helper');
  await mkdir(project, { recursive: true });
...
test('inspect fails clearly when rendered skill names collide', async () => {
  const fixture = await mkdtemp(path.join(tmpdir(), 'caddie-collision-'));
  const user = path.join(fixture, 'user');
  const project = path.join(fixture, 'project');
  await skill(path.join(user, 'source', 'first'), 'same-name');
  await skill(path.join(project, 'source', 'second'), 'same-name');
  const userManifest = path.join(user, 'caddie.json');
  await json(userManifest, manifest('user', './source', 'first'));
  await json(path.join(project, 'caddie.json'), manifest('project', './source', 'second'));
  ...
  assert.equal(envelope.error.code, 'skill-name-collision');
  assert.equal(envelope.error.disposition, 'invalid');
  assert.equal(envelope.error.details.selections.length, 2);
});
