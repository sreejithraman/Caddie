...
function reservationPath() {
  return path.join(path.resolve(os.homedir()), '.agents', '.caddie', 'user-operation.json');
}

function journalPath(plan) {
  return path.join(path.resolve(plan.scope.root), '.agents', '.caddie', 'operation-journal.json');
}
...
async function verifyAccess(plan) {
  const candidate = reservationPath();
...
async function abandonIfUnjournaled(plan) {
  ...
  if (!await exists(current.journalPath)) await fs.rm(reservationPath(), { force: true });
}

async function writeReservation(plan, phase) {
  await writeJsonAtomic(reservationPath(), {
    version: 1,
    phase,
