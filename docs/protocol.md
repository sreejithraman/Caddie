...
`skill-rename` accepts one or more disjoint `{from, to}` pairs. Each identity binds `name`, `source`, and `path`. The request also supplies the exact final Caddie Manifest, an optional final Caddie Lock, and one inspected destination materialization per pair. The current Manifest may contain either side of each pair; the final Manifest must contain only the new selection. Caddie derives all old file and Claude-link removals, native settings changes, and the full replacement Ledger. The resulting reconciliation plan binds old and new fingerprints in `intent.renames`, writes the Ledger last, and supports normal finish or rollback recovery.

Skill Rename covers Materialized Skills. Missing old files, links, or owned settings count as already retired when the Ledger still proves their prior ownership. Exact new Caddie-owned state may remain in place. Drift, changed owned state, overlapping pairs, and unowned destination collisions block planning. Ordinary reconciliation never removes per-skill ownership.

Each inspected scope reports `unmatchedOwnership` for Ledger entries, harness links, and harness settings with no matching current Skill Selection. These findings do not reduce evidence coverage and never cause an automatic mutation.
