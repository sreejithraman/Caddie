...
Developer builds use `app.caddie.CaddieMenuApp.dev`, `Caddie Development` in Application Support, and ad-hoc signing. The build installs a checked release made from the built app, the current Node executable, and the source Caddie Skill. The Tool uses a minimal state fixture in `Caddie Development/Developer Home`, so a developer build cannot read or change live user skills. Rebuilds preserve that fixture. Developer builds do not start at login or install app updates by default.
